...
class CompanyProfilePage(BasePage):
    def __init__(self, page):
        ...
        self._COMPANY_NAME_INPUT = self.page.locator("input[type=text].text-input")
        self._COMPANY_ADD_BTN = self.page.locator("button[x-show=addCompanyBtn]")

        self._ADD_NEW_COMPANY_MODAL = self.page.locator("div[x-show=showAddCompanyModal].relative")
        self._WITHOUT_ECP_BTN = self._ADD_NEW_COMPANY_MODAL.locator("a")

    # =========================== NEW COMPANY FORM =======================================
        self._COMPANY_INFO_FORM = self.page.locator("main form")
    ...
    def fill_company_info(self, company_name):
        ...
        with self.page.expect_response('**/account/api/company_api/') as response:
