...
### Если случайно закоммитились не в той ветке

Например, коммиты сделаны в `main`, а пушить в него нельзя — нужно перенести их в отдельную ветку:

```bash
git checkout -b upgrade_framework      # создать ветку и сразу перейти (коммиты остаются в ней)
git branch -f main origin/main         # передвинуть main к origin, не переключаясь на него
git push -u origin upgrade_framework
```

Важно: сначала создаём новую ветку и только потом двигаем `main` — иначе коммиты повиснут без ссылок (доставать придётся через `git reflog`). Если в рабочей директории есть незакоммиченные изменения, перед этим сделайте `git stash` или коммит.
...
```bash
git restore -- .        # откатить изменения в файлах
